    ...
    <section id="team" className="bg-white">
      <div className="mx-auto max-w-6xl px-4 py-16">
        <div className="mb-16 text-center">
          ...
          <h2 className="mt-4 text-4xl font-bold text-[#1F2937]">Meet the Team</h2>
          <p className="mt-3 text-lg text-[#1F2937]/80">Veteran professionals translating operational experience into instruction.</p>
        </div>
        <div className="grid gap-8 md:grid-cols-3">
          {team.map((member) => (
            <article key={member.name} className="rounded-2xl border border-[#E5E7EB] p-6 text-center shadow-sm">
              <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-[#F9FAFB] text-2xl font-semibold text-[#B8860B]">
                {member.name
                  .split(" ")
                  .map((part) => part[0])
                  .join("")
              ...
              <p className="mt-4 text-[#1F2937]/80">{member.bio}</p>
            </article>
          ))}
        </div>
      </div>
